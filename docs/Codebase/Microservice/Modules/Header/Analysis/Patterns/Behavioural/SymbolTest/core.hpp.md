...
Quick summary: This slice shows the first file-local stage for behavioural_symbol_test.hpp and keeps the diagram scoped to this code unit.
...
flowchart TD
    ...
    N6["Handle behavioural symbol test to text"]
    N7["Declare call"]
    N8["Defer body"]
...
Quick summary: This slice shows the first local decision path for behavioural_symbol_test.hpp after setup.
Why this is separate: behavioural_symbol_test.hpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
...
- build_behavioural_symbol_test_tree(): Declare a callable contract and let implementation files define the runtime body
- behavioural_symbol_test_to_text(): Declare a callable contract and let implementation files define the runtime body
...
flowchart TD
    ...
    N0["Create behavioural symbol test tree"]
    N1["Declare call"]
    N2["Defer body"]
    N3["Hand back"]
    End["Return"]
