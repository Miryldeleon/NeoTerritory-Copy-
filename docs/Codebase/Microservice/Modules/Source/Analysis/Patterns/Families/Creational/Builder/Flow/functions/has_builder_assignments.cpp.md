...
Quick summary: This slice shows the first file-local stage for has_builder_assignments.cpp and keeps the diagram scoped to this code unit.
...
flowchart TD
    ...
    N1["Handle has builder assignments"]
...
Quick summary: This slice shows the first local decision path for has_builder_assignments.cpp after setup.
Why this is separate: has_builder_assignments.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
    N0["Return"]
