...
Quick summary: This slice shows the first file-local stage for statement_after_condition.cpp and keeps the diagram scoped to this code unit.
...
Quick summary: This slice shows the first local decision path for statement_after_condition.cpp after setup.
Why this is separate: statement_after_condition.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
