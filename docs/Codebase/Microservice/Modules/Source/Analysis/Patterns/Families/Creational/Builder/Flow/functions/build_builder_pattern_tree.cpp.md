...
Quick summary: This slice shows the first file-local stage for build_builder_pattern_tree.cpp and keeps the diagram scoped to this code unit.
...
flowchart TD
    ...
    N1["Create builder pattern tree"]
...
Quick summary: This slice shows the first local decision path for build_builder_pattern_tree.cpp after setup.
Why this is separate: build_builder_pattern_tree.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
