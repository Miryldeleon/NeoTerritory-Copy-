...
Quick summary: This slice shows the first file-local stage for build_source_type_skeleton_lines.cpp and keeps the diagram scoped to this code unit.
...
flowchart TD
    ...
    N1["Create source type skeleton lines"]
...
Quick summary: This slice shows the first local decision path for build_source_type_skeleton_lines.cpp after setup.
Why this is separate: build_source_type_skeleton_lines.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
