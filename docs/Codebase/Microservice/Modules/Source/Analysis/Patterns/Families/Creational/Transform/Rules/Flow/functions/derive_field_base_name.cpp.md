...
Inside the body, it mainly handles store local findings, normalize raw text before later parsing, connect local structures, and walk the local collection.
...
Quick summary: This slice shows the first file-local stage for derive_field_base_name.cpp and keeps the diagram scoped to this code unit.
...
Quick summary: This slice shows the first local decision path for derive_field_base_name.cpp after setup.
Why this is separate: derive_field_base_name.cpp has multiple branches, loops, or stage changes, so this section is split out to keep one major intent visible at a time instead of forcing one oversized diagram.
```mermaid
flowchart TD
